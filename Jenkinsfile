pipeline { 
    ...
    stages {
        ...
        stage('Build Docker') {
            ...
            steps {
                // Add grep because the default grep of the node:22.12-alpine3.21 image does not have the -P flag
                sh 'apk add grep docker' // TODO: Test if adding docker is neccessary here. It is already added in test stage
				script {
                    // Based on:

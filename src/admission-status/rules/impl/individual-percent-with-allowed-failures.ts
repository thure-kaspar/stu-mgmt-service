...
export class IndividualPercentWithAllowedFailuresImpl extends IndividualPercentWithAllowedFailures {
	...
	constructor(rule: Partial<IndividualPercentWithAllowedFailures>, assignments: AssignmentDto[]) {
		...
		this.roundAchievedPercent = RoundingMethod(
	 ...
	 */
	private studentFailedAssignment(
		achievedPoints: Map<AssignmentId, number>,
		assignmentId: AssignmentId
	): boolean {
		const achievedPercent = Percent(
			achievedPoints.get(assignmentId) ?? 0,
			this.requiredPointsToPass.get(assignmentId)
		);
	...
	private mapAchievedPointsToAssignment(
		relevantAssessments: AssessmentDto[]
	): Map<string, number> {
